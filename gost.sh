...
function Install_ct() {
  ...
  if [[ ${addyn} == [Yy] ]]; then
    ...
    wget --no-check-certificate https://ghproxy.com/https://raw.githubusercontent.com/Starktomy/Multi-EasyGost/master/gost.service && chmod -R 777 gost.service && mv gost.service /usr/lib/systemd/system
    mkdir /etc/gost && wget --no-check-certificate https://ghproxy.com/https://raw.githubusercontent.com/Starktomy/Multi-EasyGost/master/config.json && mv config.json /etc/gost && chmod -R 777 /etc/gost
  else
    rm -rf gost-linux-"$bit"-"$ct_new_ver".gz
